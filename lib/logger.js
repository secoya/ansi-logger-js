...
AnsiLogger = (function() {
  ...
  AnsiLogger.prototype.setColors = function(colorMap) {
    var color, level, _ref, _results;
    _results = [];
    for (level in colorMap) {
      color = colorMap[level];
      if ((_ref = (level = level.toUpperCase())) === "LOG" || _ref === "INFO" || _ref === "DEBUG" || _ref === "VERBOSE" || _ref === "WARN" || _ref === "ERROR" || _ref === "SUCCESS" || _ref === "TITLE" || _ref === "TIME") {
        _results.push(this["" + level + "_COLOR"] = color);
      } else {
  ...
  AnsiLogger.prototype.print = function(msg, loglevel, color) {
    ...
    if ((loglevel & AnsiLogger.prototype.ERROR_MASK) === AnsiLogger.prototype.ERROR_MASK) {
      return this.options.outputters.err(str);
    } else {
      return this.options.outputters.out(str);
    }
  };
  ...
  AnsiLogger.prototype.formatTime = function(time) {
    ...
    return "[" + this.colorize(moment(time).format(this.options['timeformat']), this.TIME_COLOR) + "]";
  };
